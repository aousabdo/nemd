...
differentiable signal-adaptive filter bank whose architecture
guarantees three properties by construction: exact reconstruction of
the input, a Parseval--Cauchy--Schwarz upper bound on total component
energy, and non-negative bandpass filter responses. Pairwise
orthogonality is a corollary that becomes exact as the softmax
temperature tends to zero. A neural analyzer maps the input spectrum
to $K$ log-responses, a softmax over the $K$ dimension produces a
...
additional loss was added. On a three-class classification benchmark
(5 seeds, $3$ SNR levels), end-to-end N-EMD matches a capacity-matched
VMD$+$MLP baseline at SNR $3$~dB ($94.2{\pm}2.9$ vs.\ $92.7{\pm}0.8$
percent) and loses at $10$ and $20$~dB with high training-run
variance, so N-EMD's advantage is narrow and concentrated in the
low-SNR regime; the architectural guarantees transfer unchanged to
held-out distributions and to real pupillometry data, where neither
N-EMD nor any baseline (including fixed VMD and a raw CNN) beats
majority-class prediction --- a dataset-level rather than
method-level null.
\end{abstract}
...
decomposition. (2)~\emph{EMD+MLP (fixed)}: EMD $\to$ 9 features (log-
energy, spectral centroid, bandwidth per IMF) $\to$ MLP
$9{\to}64{\to}64{\to}3$ ($5$\,k params). (3)~\emph{VMD+MLP (small)}:
same with VMD. (4)~\emph{VMD+MLP (big)}: VMD features, larger MLP
$9{\to}320{\to}320{\to}3$ matching N-EMD's capacity ($107$\,k
params); the capacity-match baseline requested by reviewers.
...
(7)~\emph{N-EMD (scratch, e2e)}: the architecture of
Section~\ref{sec:method} trained end-to-end from random initialisation
with cross-entropy plus a $0.1{\times}$ physics regulariser ($130$\,k
params). Feature extraction is differentiable in PyTorch; gradients
propagate through every learned frontend. Training: $50$ epochs with
Adam. Cheap baselines: $5$ seeds; end-to-end N-EMD: $3$ seeds.
...
\begin{table}[t]
\centering
\caption{Three-class classification, multi-seed. Cheap baselines: 5
seeds; N-EMD scratch: 3 seeds; N-EMD pretrained: 2 seeds (SNRs 3, 20)
and $\lambda_{\text{phys}}=0.1$ row of the ablation at SNR 10.
``VMD+MLP (big)'' is the capacity-matched VMD baseline ($107$\,k
params). Bold marks the best non-raw-CNN pipeline in each SNR block.}
\label{tab:snr-sweep}
...
$3$ dB & Raw CNN                           & $99.17$\,\%                         & $81$\,k  \\
       ...
       & N-EMD pretrained (e2e)            & $82.33 \pm 0.83$                    & $130$\,k \\
       & N-EMD scratch (e2e)               & $\mathbf{94.17 \pm 2.90}$           & $130$\,k \\
\midrule
$10$ dB & Raw CNN                          & $99.83$\,\%                         & $81$\,k  \\
       ...
       & N-EMD pretrained (e2e)            & $89.67 \pm 1.33$                    & $130$\,k \\
       & N-EMD scratch (e2e)               & $72.44 \pm 6.81$                    & $130$\,k \\
\midrule
$20$ dB & Raw CNN                          & $99.67$\,\%                         & $81$\,k  \\
       ...
       & N-EMD pretrained (e2e)            & $78.83 \pm 4.67$                    & $130$\,k \\
       & N-EMD scratch (e2e)               & $88.78 \pm 9.03$                    & $130$\,k \\
\bottomrule
\end{tabular}
\end{table}

\emph{(i) Capacity matters.} The original small-MLP VMD baseline
reaches $83.2 \pm 3.2$\,\% at $3$ dB; feeding the same VMD features
into a capacity-matched MLP (VMD+MLP (big), $107$\,k parameters) adds
$+9.4$ percentage points at no cost to the decomposition. The bulk of
the gap between a fixed-preprocessing pipeline and an end-to-end
learned one can be closed by head capacity alone, not by learning the
frontend. Our earlier single-seed table understated this.
...
\emph{(iii) N-EMD's task-aware advantage is real but narrow.} At
$3$ dB, N-EMD (scratch, e2e) reaches $94.2 \pm 2.9$\,\%, versus
$92.7 \pm 0.8$\,\% for the capacity-matched VMD+MLP (big) --- a
$+1.5$~pp lead whose error bars overlap. Against the non-capacity-
matched small-MLP VMD baseline, the lead is $+11$ pp. At $10$ and
$20$ dB, N-EMD underperforms the capacity-matched baseline by
$23$ and $7$ pp respectively, and the scratch-variant accuracy has
much larger run-to-run variance (std $6.8$ and $9.0$\,\%) than any
fixed-preprocessing baseline. The task-aware advantage exists only
in the low-SNR regime, is of modest magnitude against a properly-
tuned baseline, and comes with a training-instability penalty at
higher SNR that is only partly understood.

\paragraph{Physics-regulariser ablation and training stability.}
Fig.~\ref{fig:physics-ablation} sweeps
$\lambda_{\text{phys}} \in \{0, 0.01, 0.1, 1.0\}$ on two seeds at
SNR $10$ dB with pretrained initialisation. The default
$\lambda = 0.1$ is the accuracy sweet spot ($89.7{\pm}1.3$\,\%);
removing the regulariser ($\lambda = 0$) drops to $79.7$\,\% and
over-weighting it ($\lambda = 1$) drops to $84.2$\,\%. The physics
regulariser helps at its default weight. We also find a nontrivial
\emph{initialisation} effect that was not apparent from single-seed
runs: pretraining stabilises training in the SNR $10$ dB regime
(std $1.3$\,\% vs.\ scratch's $6.8$\,\%) but underperforms scratch
at SNR $3$ and $20$ dB. Neither initialisation dominates across all
SNR levels. Both show substantially higher run-to-run variance than
any fixed-preprocessing baseline, which we read as a real training
stability limitation of the end-to-end approach on this task.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figs/physics_ablation.png}
\caption{Physics-regulariser weight sweep for the pretrained N-EMD
classifier at SNR $10$ dB (two seeds; error bars are one standard
deviation). The default $\lambda_{\text{phys}} = 0.1$ is the sweet
spot; neither removing the regulariser nor over-weighting it
improves accuracy.}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figs/snr_sweep_accuracy_ci.png}
\caption{Multi-seed classification accuracy versus SNR (cheap
baselines: 5 seeds; N-EMD: 3 seeds; error bars are one standard
deviation). The capacity-matched VMD+MLP (big) narrows the end-to-end
N-EMD advantage at $3$ dB to $+1.5$ pp with overlapping error bars,
and dominates at $10$ and $20$ dB. Generic learnable filter banks
(Mel-FB, SincNet) underperform decomposition-specific preprocessing.
Chance ($1/3$) in dashed.}
\label{fig:snr-sweep}
\end{figure}
...
On three canonical classification formulations --- 2-class (control
vs.\ memory task, majority $67$\,\%), 3-class (attention condition),
6-class (condition $\times$ subject-group) --- none of the tested
pipelines reaches above the majority-class baseline by a reliable
margin under a subject-level split. 2-class accuracy across
$\{$EMD$+$MLP, VMD$+$MLP, VMD$+$MLP (big), Mel-FB$+$MLP$\}$ is
$0.667 \pm 0.001$ over three seeds, with balanced accuracy pinned at
$0.500$ --- every pipeline learns to predict majority. For 3-class,
accuracies across five pipelines are within $\pm 4$ pp of chance
$(1/3)$; 6-class is similarly uninformative. The null is consistent
across all pipelines and tasks --- a strong indication that single
...
information in this dataset. We therefore read this experiment as a
decomposition-quality transfer result rather than a downstream-task
result: N-EMD's architectural guarantees hold on real biomedical data
(Fig.~\ref{fig:pupil-decomp} and the OI/ER numbers above) without
retraining, but separating this set of experimental conditions is not
doable by decomposition-then-classify with any of the methods we
tested. A real-world task win is a natural next step and is outside
...
\paragraph{SNR-regime dependence and capacity-match honesty.}
The multi-seed task-aware sweep (Table~\ref{tab:snr-sweep}) is more
nuanced than a single-seed run suggested. Against the capacity-matched
VMD$+$MLP (big) baseline, N-EMD's $3$~dB advantage is only
$+1.5$ pp with overlapping error bars; against the small-MLP baseline,
it is $+11$ pp. The original $+7.8$ pp claim (from single-seed data
with a $5$\,k-parameter MLP head) was an artifact of capacity
mismatch, not only of task-aware training. At $10$ and $20$ dB,
N-EMD underperforms the capacity-matched baseline by $23$ and $7$ pp
and shows high run-to-run variance (std $6.8$ and $9.0$ pp). The
honest reading: the decomposition-specific architecture helps at low
SNR with a modest margin, loses at high SNR, and has training
stability problems at high SNR that the physics-regulariser ablation
begins to explain. N-EMD should be proposed as a complement to fixed
VMD preprocessing, not a replacement; a hybrid pipeline that uses
VMD$+$MLP by default and switches to end-to-end N-EMD below a
calibration SNR threshold is the cleanest deployment.
...
benchmark, end-to-end N-EMD matches a capacity-matched VMD+MLP
baseline at SNR $3$~dB within error bars and underperforms it at
$10$ and $20$~dB, so N-EMD's task-aware advantage is narrow and
concentrated in the low-SNR regime. Generic learnable filter banks
