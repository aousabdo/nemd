...
\end{tikzpicture}}
\caption{Architecture of N-EMD. A 1D CNN analyzer reads the magnitude
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.95\textwidth]{figs/nonstationary_crossing_chirps.png}
...
N-EMD. N-EMD separates by frequency band rather than by instantaneous
frequency, so the crossing chirps get split across two band passes ---
a known limitation of every frequency-partitioning method (see
...
\caption{Time--frequency content of EMD, VMD, and N-EMD IMFs on the
canonical three-component signal. Rows are IMFs $1$--$3$ (top to
bottom); columns are the three methods. N-EMD concentrates each IMF
