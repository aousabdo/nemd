...
\paragraph{Physics-regulariser ablation.}
The raw CNN sits at $99$\,\% across all SNR levels; it needs no
bandpass decomposition to solve this task. A pretrained-initialisation
variant of N-EMD, fine-tuned with the same $0.1{\times}$ physics
regulariser used in training, reaches only $\approx 0.72$\,\% at
$10$ and $20$ dB in single-seed runs --- substantially worse than
scratch. Fig.~\ref{fig:physics-ablation} sweeps the regulariser
weight at SNR $10$ dB over $\lambda_{\text{phys}} \in \{0, 0.01,
0.1, 1.0\}$ on two seeds. The hypothesis is that for clean-signal
inputs the physics-optimal partition is close enough to VMD's that
the regulariser actively fights the classifier; the ablation tests
whether loosening or removing it recovers performance.
