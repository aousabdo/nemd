...
capacity-matched VMD$+$MLP baseline, end-to-end N-EMD beats the
baseline at SNR $3$\,dB ($94.83 \pm 1.21$ vs.\ $92.67 \pm 0.78$\,\%,
$+2.2$~pp with non-overlapping error bars at one standard deviation)
...
\item\emph{End-to-end task-aware training.} The architecture is
  ...
  end-to-end at SNR $3$~dB beats a capacity-matched fixed-VMD baseline
  ($94.83 \pm 1.21$\,\% vs.\ $92.67 \pm 0.78$\,\%, $+2.2$~pp with
  non-overlapping error bars at one standard deviation) while
...
\item\emph{Honest SNR-regime analysis.} The end-to-end advantage is
  ...
  (big) outperforms the single-configuration N-EMD by $3.3$ and
  $3.6$~pp respectively (Section~\ref{sec:taskaware}). Task-aware
...
capacity-matched VMD baseline ($107$\,k params). ``N-EMD (default,
...
Bold marks the better of N-EMD (default) vs.\ VMD+MLP (big) in each
SNR block.}
\label{tab:snr-sweep}
...
$3$ dB & Raw CNN                           & $99.17$\,\%                         & $81$\,k  \\
       ...
       & N-EMD (default, e2e)              & $\mathbf{94.83 \pm 1.21}$           & $232$\,k \\
       & \quad ablation: small head        & $94.17 \pm 2.90$                    & $130$\,k \\
\midrule
$10$ dB & Raw CNN                          & $99.83$\,\%                         & $81$\,k  \\
       ...
       & N-EMD (default, e2e)              & $92.39 \pm 4.05$                    & $232$\,k \\
       & \quad ablation: small head        & $72.44 \pm 6.81$                    & $130$\,k \\
       & \quad ablation: $\lambda_{\text{phys}}{=}0$ & $97.28 \pm 0.97$          & $130$\,k \\
\midrule
$20$ dB & Raw CNN                          & $99.67$\,\%                         & $81$\,k  \\
       ...
       & N-EMD (default, e2e)              & $92.28 \pm 1.85$                    & $232$\,k \\
       & \quad ablation: small head        & $88.78 \pm 9.03$                    & $130$\,k \\
       & \quad ablation: $\lambda_{\text{phys}}{=}0$ & $93.22 \pm 6.00$          & $130$\,k \\
\bottomrule
\end{tabular}
\end{table}
...
\emph{(iii) Default-configuration N-EMD beats the capacity-matched
baseline at SNR $3$ dB, loses in the near-ceiling regime.} The default
N-EMD classifier uses the scratch-initialised analyzer, capacity-
matched MLP head ($107$\,k), and the training-time physics regulariser
weight ($\lambda_{\text{phys}} = 0.1$). It reaches
$94.83 \pm 1.21$\,\% at SNR $3$ dB, $+2.2$\,pp over VMD$+$MLP (big)
with non-overlapping error bars. At $10$ and $20$ dB it reaches
$92.39$ and $92.28$\,\%, $-3.3$ and $-3.6$\,pp behind the baseline
where the raw CNN itself is above $99$\,\%. The task-aware advantage
is genuine at low SNR and small in the near-ceiling regime.

\emph{(iv) Ablations: head capacity and physics-regulariser weight.}
...
at $3$ dB but suffers from training instability at $10$ and $20$ dB
(std $6.8$ and $9.0$\,\%, versus $1.3$ and $1.9$\,\% for the default);
matching the head capacity to the VMD$+$MLP (big) baseline eliminates
most of this instability. Removing the physics regulariser
($\lambda_{\text{phys}} = 0$) on top of the small-head scratch variant
at SNR $10$ dB recovers $+24.8$\,pp and yields the highest single-SNR
N-EMD number we observed ($97.28 \pm 0.97$\,\%, $+1.6$ pp over the
baseline); the regulariser anchors the filter bank to a
decomposition-optimal configuration that is not task-optimal when the
task gradient alone has enough signal at clean SNR. We report this
result as an ablation rather than as the default: it requires a
per-SNR hyperparameter choice and so converts the paper from proposing
a single method to proposing a routed family, which we do not want to
do here. The default configuration above is the one we recommend.
...
deviation). At SNR $3$\,dB, N-EMD (big head) beats the capacity-matched
VMD$+$MLP (big) by $+2.2$\,pp with non-overlapping error bars; at $10$
and $20$\,dB, VMD$+$MLP (big) leads by $3.3$ and $3.6$\,pp in the
near-ceiling regime. Generic learnable filter banks (Mel-FB, SincNet)
underperform decomposition-specific preprocessing at every SNR. The
$\lambda_{\text{phys}} = 0$ scratch variant (Section~\ref{sec:taskaware}
paragraph iv) closes the gap at $10$\,dB but is not part of the default
pipeline. Chance ($1/3$) in dashed.}
\label{fig:snr-sweep}
\end{figure*}
...
tested pipelines --- including the raw CNN that reaches $99$\,\% on
the synthetic benchmark --- exceeds the majority-class baseline by a reliable margin: 2-class accuracy is
...
Against the capacity-matched VMD$+$MLP (big) baseline, the single-
configuration N-EMD (scratch, big MLP head, default
$\lambda_{\text{phys}} = 0.1$) beats the baseline at SNR $3$\,dB
($+2.2$\,pp with non-overlapping error bars) and loses by $3.3$ and
$3.6$\,pp at $10$ and $20$\,dB, where every decomposition-based method
sits above $92$\,\%. The end-to-end advantage is concentrated where
task-aware training matters most; at clean SNR the fixed-preprocessing
pipeline is already near the raw-CNN ceiling and there is little
headroom for the learned frontend. N-EMD$+$VMD$+$MLP (big) are
complementary rather than substitutes; a practical pipeline would use
VMD$+$MLP (big) by default and switch to end-to-end N-EMD below a
calibration SNR threshold (around $5$\,dB in our sweep). The ablation
in paragraph (iv) of Section~\ref{sec:taskaware} shows that further
tuning (notably $\lambda_{\text{phys}} = 0$ for scratch at clean SNR)
can narrow or reverse the $10$--$20$\,dB gap but adds a configuration
knob; we do not include it in the default pipeline.
...
benchmark with a capacity-matched VMD$+$MLP baseline, the default
single-configuration N-EMD classifier beats the baseline at SNR
$3$\,dB ($+2.2$~pp, non-overlapping error bars at one standard
deviation) and loses by $3.3$ and $3.6$~pp at $10$ and $20$\,dB,
where every method including the raw CNN sits above $92$\,\%. A
...
repository is private at the time of submission; reviewer access can
be granted on request, and the repository will be made public upon
acceptance. All experiments
