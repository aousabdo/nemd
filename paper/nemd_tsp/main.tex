...
EMD, VMD, and N-EMD --- every frequency-partitioning decomposition
...
$9{\to}64{\to}64{\to}3$ ($5$\,k params). A richer feature set
(e.g.\ Hilbert-instantaneous-frequency statistics, higher moments)
would give a stronger fixed baseline; we use the minimal set above
to keep the frontend-vs-head factorisation clean. The capacity-match
baseline below operates on the same features. (3)~\emph{VMD+MLP (small)}:
...
$5$ seeds; all N-EMD rows: $3$ seeds. ``VMD+MLP (big)'' is the
capacity-matched VMD baseline ($107$\,k params); ``N-EMD (e2e, big
head)'' matches its head capacity. All N-EMD rows use scratch
initialisation. Bold marks the best non-raw-CNN pipeline in each SNR
block; pretrained-initialisation numbers appear only in the
physics-regulariser discussion.}
\label{tab:snr-sweep}
...
\emph{(iv) The frontend and the loss interact with SNR.} Two targeted
ablations (Table~\ref{tab:snr-sweep}, last three rows of each block)
explain the result. A capacity-matched head (N-EMD big head,
$107$\,k MLP = $232$\,k total) improves over the default small head
by $+0.7$, $+19.9$, and $+3.5$ pp and collapses the $20$\,dB variance
from $9.0$\,\% to $1.9$\,\%. Removing the physics regulariser
($\lambda_{\text{phys}}{=}0$) on scratch training adds a further
$+24.8$ pp at $10$ dB and reduces std to $1.0$\,\%. The regulariser
anchors the filter bank to a decomposition-optimal configuration that
is not task-optimal when the task gradient alone has enough signal.
The practical recommendation is thus $\lambda_{\text{phys}}{=}0$ when
training scratch at SNR $\gtrsim 10$\,dB, and the default
$\lambda_{\text{phys}}{=}0.1$ when fine-tuning from the physics-only
checkpoint (Fig.~\ref{fig:physics-ablation}).
...
mismatch, not only of task-aware training. With the right configuration per SNR, end-to-end N-EMD wins at $3$
and $10$ dB against the capacity-matched baseline with non-overlapping
error bars ($+2.2$ and $+1.6$ pp respectively) and loses by $3.5$ pp
at $20$ dB where every method sits above $92$\,\%. The physics
regulariser is the key hyperparameter: default $\lambda = 0.1$ at
clean SNR destabilises scratch training, but $\lambda = 0$ at $10$
dB recovers $+24.8$ pp on scratch. An oracle that selects the best
configuration per SNR averages $95.98$\,\% across $3$, $10$, and
$20$ dB, versus $94.72$\,\% for pure VMD$+$MLP (big) --- a $+1.26$
pp headroom that a calibration-based SNR router could largely realise
in deployment. We suggest the following default: at SNR $\lesssim 10$
dB use N-EMD end-to-end; at $> 10$ dB use VMD$+$MLP (big).
