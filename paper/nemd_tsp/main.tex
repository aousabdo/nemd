...
have been sparse. Where they exist, they tend to drop one of the
desiderata: either the decomposition is implicit inside the network
(no bandpass structure that can be plotted or related to physical
frequencies), or the reconstruction identity is approximate (a hidden
residual soaks up the error), or task-awareness is absent. To the best
of our knowledge, no prior work provides a differentiable decomposition
with \emph{exact} reconstruction, non-negative bandpass filter
responses, and task-aware training jointly.

\subsection{Differentiable Signal Processing}

The differentiable DSP line of work, notably DDSP~\cite{engel2020ddsp},
incorporates explicit signal-processing structure --- oscillators,
filters, noise generators --- into neural networks, improving
interpretability and sample efficiency over end-to-end black boxes.
SincNet~\cite{ravanelli2018sincnet} parameterises the first layer of a
speech-recognition model as a bank of sinc-windowed bandpass filters
with learnable cutoffs. Pariente et al.~\cite{pariente2020filterbank}
showed that a fully learnable analysis-synthesis filter bank improves
on STFT baselines for speech separation. N-EMD sits in this same
tradition. Our architectural claim --- that a softmax partition over
$K$ learned responses gives a sum-to-one filter bank with exact
reconstruction --- is, to our knowledge, new as a parameterisation for
signal decomposition. The softmax-as-gating construction has a long
history in mixture-of-experts models~\cite{jacobs1991moe}; what is new
here is the realisation that the same algebra turns a generic
frequency-domain network into a decomposition method with structural
guarantees.
...
All experiments use the same trained checkpoint
(\texttt{checkpoints\_p25b\_v3}), $K = 3$, $d = 64$ hidden channels,
three residual blocks --- the architecture of Section~\ref{sec:method}.
Baselines (EMD, VMD) use standard reference implementations with
$K = 3$ modes to match.
...
initialised from the \texttt{p25b\_v3} checkpoint and the entire
...
Section~\ref{subsec:generalization}) becomes the bottleneck. The
practical implication is that task-aware learned decomposition is most
valuable where fixed preprocessing degrades most: in noisy operating
conditions.
...
\paragraph{Classification is not informative.}
...
The decomposition methods transfer cleanly; the classification task
itself, on pupil response alone without additional covariates,
appears not to be separable by any of the tested pipelines. The
positive result is that N-EMD's guarantees hold on real data; the
...
(iii)~Domain adaptation: a companion astrophysics paper, currently in
preparation, applies the method to pulsar timing residuals with a
fine-tuning step on observation-specific red noise; the partition-of-
unity structure lets that fine-tuning preserve reconstruction and
energy bounds by construction. (iv)~Probabilistic extensions in which
the softmax partition is replaced by a distribution over partitions,
which would quantify decomposition uncertainty in low-SNR regimes.
...
\begin{table}[h]
\centering
\caption{Training hyperparameters for the \texttt{p25b\_v3} checkpoint.}
\label{tab:hparams}
\begin{tabular}{lr}
\toprule
