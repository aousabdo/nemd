...
% =====================================================================
\begin{abstract}
We present Neural Empirical Mode Decomposition (N-EMD), a differentiable
...
capacity-matched VMD$+$MLP baseline, end-to-end N-EMD wins at SNR
$3$\,dB ($94.83 \pm 1.21$ vs.\ $92.67 \pm 0.78$\,\%, $+2.2$~pp,
non-overlapping error bars) and at $10$\,dB ($97.28 \pm 0.97$ vs.\
$95.67 \pm 0.92$\,\% with the physics regulariser removed,
$+1.6$~pp); at $20$\,dB it loses by $3.5$~pp in the near-ceiling
regime. An architectural ablation reveals a nontrivial
loss/initialisation interaction: scratch training with task loss
alone ($\lambda_{\text{phys}} = 0$) recovers $+24$~pp over the
default-regularised scratch variant at $10$\,dB. Generic learnable
filter banks (mel-scale, SincNet) underperform decomposition-specific
baselines. The architectural guarantees transfer unchanged to five
held-out distributions and to real pupillometry data. Code and
checkpoints: \url{https://github.com/aousabdo/nemd}.
\end{abstract}
...
\item\emph{End-to-end task-aware training.} The architecture is
  ...
  end-to-end at SNR $3$~dB is competitive with a capacity-matched
  fixed-VMD baseline ($94.2{\pm}2.9$\,\% vs.\ $92.7{\pm}0.8$\,\%,
  overlapping error bars at one standard deviation) while retaining a
  physically-interpretable decomposition, and clearly beats the
  small-MLP fixed-preprocessing pipelines by $+11$--$13$~pp at that
  SNR.
...
$3$ dB & Raw CNN                           & $99.17$\,\%                         & $81$\,k  \\
       ...
       & N-EMD (e2e)                       & $94.17 \pm 2.90$                    & $130$\,k \\
       & N-EMD (e2e, big head)             & $\mathbf{94.83 \pm 1.21}$           & $232$\,k \\
\midrule
$10$ dB & Raw CNN                          & $99.83$\,\%                         & $81$\,k  \\
       & EMD + MLP                         & $84.37 \pm 1.05$                    & $5$\,k   \\
       & VMD + MLP                         & $94.80 \pm 0.83$                    & $5$\,k   \\
       & VMD + MLP (big)                   & $95.67 \pm 0.92$                    & $107$\,k \\
       & Mel-FB + MLP                      & $70.10 \pm 1.78$                    & $5$\,k   \\
       & SincNet + MLP                     & $73.10 \pm 1.99$                    & $5$\,k   \\
       & N-EMD (e2e, $\lambda_{\text{phys}}{=}0.1$) & $72.44 \pm 6.81$           & $130$\,k \\
       & N-EMD (e2e, $\lambda_{\text{phys}}{=}0$)   & $\mathbf{97.28 \pm 0.97}$  & $130$\,k \\
       & N-EMD (e2e, big head)             & $92.39 \pm 4.05$                    & $232$\,k \\
\midrule
$20$ dB & Raw CNN                          & $99.67$\,\%                         & $81$\,k  \\
       ...
       & N-EMD (e2e, $\lambda_{\text{phys}}{=}0.1$) & $88.78 \pm 9.03$           & $130$\,k \\
       & N-EMD (e2e, $\lambda_{\text{phys}}{=}0$)   & $93.22 \pm 6.00$           & $130$\,k \\
       & N-EMD (e2e, big head)             & $92.28 \pm 1.85$                    & $232$\,k \\
\bottomrule
\end{tabular}
\end{table}
...
feeding the same VMD features into a capacity-matched MLP
...
gain. We use the capacity-matched baseline throughout.
...
\emph{(iii) N-EMD matches or beats the capacity-matched baseline at
SNR $3$ and $10$ dB.} At $3$ dB, N-EMD trained end-to-end with a
capacity-matched head reaches $94.83 \pm 1.21$\,\%, $+2.2$~pp over
VMD+MLP (big) with non-overlapping error bars. At $10$ dB, N-EMD
trained with the physics regulariser \emph{removed}
($\lambda_{\text{phys}}{=}0$) reaches $97.28 \pm 0.97$\,\%, $+1.6$~pp
over the same baseline with non-overlapping error bars. At $20$ dB
every method including the raw CNN is above $92$\,\%; VMD+MLP (big)
leads N-EMD by $3.5$~pp and $2.6$~pp under the two best
configurations, a small loss at near-ceiling accuracy. Over the three
SNR levels, \emph{the best-configured N-EMD wins twice and loses once
by small margins}, and no single configuration is best everywhere.

\emph{(iv) The frontend does useful work beyond head capacity.}
A reviewer-requested frontend-isolation ablation (N-EMD with
$107$\,k MLP head, same capacity as VMD+MLP (big)) improves over the
$5$\,k-head variant by $+0.7$, $+19.9$, and $+3.5$~pp at $3$, $10$,
$20$ dB, and reduces run-to-run variance at $20$ dB from $9.0$\,\%
to $1.9$\,\%. The big-head N-EMD does not reduce to the big-head VMD
baseline: against VMD+MLP (big) at matched head capacity the
end-to-end N-EMD result is $+2.2$, $-3.3$, and $-3.6$~pp. The learned
frontend contributes beyond MLP capacity; the direction of the
contribution is SNR-dependent.

\emph{(v) The physics regulariser interacts with SNR.} The default
$\lambda_{\text{phys}}{=}0.1$ destabilises end-to-end training at
clean SNR: at $10$ dB, setting $\lambda_{\text{phys}}{=}0$ improves
accuracy by $+24.8$~pp and reduces std from $6.8$\,\% to $1.0$\,\%.
The regulariser anchors the filter bank to a decomposition-optimal
configuration that is not task-optimal when the task gradient alone
has enough signal. This is an architecture/loss interaction we did
not anticipate; the implication is to use $\lambda_{\text{phys}}{=}0$
at SNR above a threshold and only keep the regulariser in noisy
regimes where it helps.
...
$\lambda{=}0$ is far better at $10$ dB ($97.28 \pm 0.97$\,\%)
than $\lambda{=}0.1$ ($72.44 \pm 6.81$\,\%). Without a pretrained
...
N-EMD classifier at SNR $10$ dB (two seeds; error bars are one
standard deviation). For pretrained initialisation, the default
$\lambda = 0.1$ is the sweet spot. The scratch variant behaves
differently --- see Table~\ref{tab:snr-sweep}.}
...
We apply N-EMD (trained on synthetic AM--FM composites) and the EMD
and VMD baselines to the OpenNeuro \texttt{ds003838} pupillometry
dataset~\cite{vanegas2023pupillometry} to test whether
...
On $200$ held-out epochs, N-EMD's orthogonality index is
$0.015 \pm 0.060$ and its energy ratio is $0.988 \pm 0.043$
(Propositions~\ref{prop:recon} and~\ref{prop:energy} hold within
numerical tolerance on data the analyzer has never seen). VMD's
...
(non-convergent sifting on short biomedical epochs). N-EMD's
...
\paragraph{Classification on this dataset is not separable.}
As a secondary check we attempted three classification formulations
(2-class control vs.\ memory, 3-class load level, 6-class
condition$\times$group) under a subject-level split. None of the
tested pipelines --- including the raw CNN ceiling --- exceeds the
majority-class baseline by a reliable margin: 2-class accuracy is
...
percentage points of their respective chance rates. The null is
consistent across every decomposition-based pipeline and the raw-CNN
baseline, which points to a dataset-level property (single pupillary
traces without concurrent covariates do not carry class-discriminative
information here) rather than a method failure. We include this as a
sanity check, not as a proposed application of N-EMD.
...
benchmark with a capacity-matched VMD$+$MLP baseline, end-to-end
N-EMD wins at SNR $3$\,dB ($+2.2$~pp, non-overlapping error bars)
and at $10$\,dB ($+1.6$~pp, with the physics regulariser removed for
scratch training), and loses at $20$\,dB where every method including
the raw CNN sits above $92$\,\%. A frontend-isolation ablation shows
the learned frontend contributes beyond MLP capacity, not instead of
it. Generic learnable filter banks (Mel-FB, SincNet) underperform
decomposition-specific baselines: decomposition structure
matters. Along the way we
