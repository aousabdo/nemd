...
\begin{table}[t]
\centering
\caption{Three-class classification, multi-seed. 5 seeds for the
cheap baselines; 3 seeds for N-EMD. ``VMD+MLP (big)'' is the
capacity-matched VMD baseline ($107$\,k parameters). Bold marks the
best non-raw-CNN pipeline in each SNR block.}
\label{tab:snr-sweep}
...
An earlier single-seed run of the pretrained-initialisation variant
at SNR $10$ dB produced an anomalously low $72$\,\% accuracy, which
we initially attributed to a regulariser--classifier conflict.
Fig.~\ref{fig:physics-ablation} sweeps
$\lambda_{\text{phys}} \in \{0, 0.01, 0.1, 1.0\}$ on two seeds and
disproves that hypothesis: the default $\lambda = 0.1$ is the
accuracy sweet spot ($89.7{\pm}1.3$\,\%); removing the regulariser
($\lambda = 0$) and over-weighting it ($\lambda = 1$) both degrade
performance. The original low number was a single-seed outlier. Two
substantive findings remain: (a) the physics regulariser is helpful,
not harmful, at its default weight; (b) the scratch variant has
substantially higher run-to-run variance at higher SNR (std
$6.8$--$9.0$\,\%) than the pretrained variant, so pretraining on
synthetic decomposition data stabilises task-aware fine-tuning.
Pretrained$+$fine-tuned is the recommended training regime.
