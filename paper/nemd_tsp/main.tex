...
head-matched VMD$+$MLP baseline, end-to-end N-EMD beats the
baseline at SNR $3$\,dB ($94.83 \pm 1.21$ vs.\ $92.67 \pm 0.78$\,\%,
$+2.2$~pp over the baseline, mean $\pm$ std of three training runs)
...
\item\emph{End-to-end task-aware training.} The architecture is
  ...
  ($94.83 \pm 1.21$\,\% vs.\ $92.67 \pm 0.78$\,\%, $+2.2$~pp; mean
...
$9{\to}320{\to}320{\to}3$ matching the N-EMD classifier's
...
head-matched VMD baseline ($107$\,k params). ``N-EMD (default,
...
\emph{(iii) Default-configuration N-EMD beats the head-matched
baseline at SNR $3$ dB, loses in the near-ceiling regime.} The default
N-EMD classifier uses the scratch-initialised analyzer, a $107$\,k-
parameter MLP head matched to the VMD$+$MLP (big) head capacity, and
the training-time physics regulariser weight
($\lambda_{\text{phys}} = 0.1$). It reaches $94.83 \pm 1.21$\,\%
at SNR $3$ dB, $+2.2$\,pp over VMD$+$MLP (big) at $92.67 \pm 0.78$
(mean $\pm$ std of three training runs). At $10$ and $20$ dB it
reaches $92.39$ and $92.28$\,\%, $3.3$ and $3.6$\,pp behind the
baseline's $95.67$ and $95.83$\,\% where the raw CNN itself is at
$99.83$ and $99.67$\,\%. The task-aware advantage is genuine at low
SNR and small in the near-ceiling regime.
...
N-EMD classifier at SNR $10$ dB. Markers are the per-seed means and
...
\caption{Multi-seed classification accuracy versus SNR. ``N-EMD
...
$5$ seeds. N-EMD: $3$ seeds. Error bars are one standard deviation.
At SNR $3$\,dB, N-EMD (default) beats VMD$+$MLP (big) by $+2.2$\,pp;
at $10$ and $20$\,dB, VMD$+$MLP (big) leads by $3.3$ and $3.6$\,pp in
...
(means $94.83$ vs.\ $92.67$, $+2.2$\,pp) and loses by $3.3$ and
$3.6$\,pp at $10$ and $20$\,dB, where every decomposition-based
...
the learned frontend. The ablation in paragraph (iv) of
Section~\ref{sec:taskaware} shows that further tuning (notably
$\lambda_{\text{phys}} = 0$ for scratch at clean SNR) can narrow or
reverse the $10$--$20$\,dB gap but adds a configuration knob; we do
not include it in the default pipeline. A hybrid deployment that
switches between VMD$+$MLP (big) and end-to-end N-EMD based on
...
$3$\,dB ($+2.2$~pp, mean $\pm$ std over three training runs) and
loses by $3.3$ and $3.6$~pp at $10$ and $20$\,dB, where every
